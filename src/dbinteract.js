...
const parseUrl = async (sessionKey) => {
    if (
        ...
        validUrl.isWebUri(document.querySelector('#url').value) &&
        ...
        await fetch('/generate-url', generateRequestOptions)
            .then((res) => res.json())
            .then((json) => {
                document.querySelector(
                    '#output'
                ).value = `https://squrl.dev/${json.urlRoute}+${sessionKey}`;
            })
            .catch((e) => {
        ...
        document.querySelector('#squrl-error').innerHTML =
            'URL is too long, even for this service.';
    } else {
    ...
    if (key.value !== '') {
        decryptUrlOptions.body = JSON.stringify({
            urlRoute: squrlUrl.value
                .split('/')
                [squrlUrl.value.split('/').length - 1].substring(0, 6),
            sessionKey: key.value,
        });
        await fetch('/decrypt-url', decryptUrlOptions)
            .then((res) => res.json())
            .then((json) => {
                decryptedUrl.value = decryptUrl(json.encryptedUrl, key.value).toString(
                    cryptoJs.enc.Utf8
                );
            })
            .catch((e) => {
        ...
        document.querySelector('#decrypter-error').innerHTML =
            'Please enter a valid Session Key or Encrypted URL.';
    }
};
