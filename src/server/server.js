...
app.get('/:urlRoute', (req, res) => {
    ...
    if (urlRoute.includes('+') && urlRoute.length === 13) {
        // hard coded: url: {urlRoute} '+' {sessionKey} = length
        if (sessionKey.length) {
            ...
            Url.findOne({ urlRoute: urlRoute.substring(0, urlRoute.indexOf('+')) }, (err, doc) => {
                if (!doc) {
                    ...
                    const nonce = new Entropy({
                        charset: charset64,
                        bits: conf.bitLength,
                    }).string(); // total: 1e4, risk: 1e6,
                    res.setHeader(
                        'Content-Security-Policy',
                        `object-src 'none'; script-src 'nonce-${nonce}' 'strict-dynamic'`
                    );
                    res.render(
                        'redirector',
                        {
        ...
        Url.findOne({ urlRoute: urlRoute }, (err, doc) => {
            if (!doc) {
                res.sendStatus(404);
            } //  There are no documents associated with this urlRoute.
            else {
    ...
    Url.findOne(
        ...
        (err, doc) => {
            if (!doc) {
                ...
                res.sendStatus(409)
                    .send('URL route was taken... odds of this happening are very slim.')
                    .send(err);
            }
        }
    );
